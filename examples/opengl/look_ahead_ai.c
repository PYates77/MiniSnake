...
int getDistance(int x, int y, int d){ // starting coordinates x,y, and distance d 
    int space = 0;
    while(x < windowWidth && x > 0 && y < windowHeight && y > 0 && !partAt(x,y)){
        ++space;
        switch(d){
            case 0:
                ++x;
    ...
    if(game->head->x != game->mouse.x || game->head->y != game->mouse.y){ //do nothing if you just ate
        if(game->head->x < game->mouse.x){
            if(invalid[0]){
                ...
                if(getDistance(game->head->x,game->head->y+1,1)>getDistance(game->head->x,game->head->y-1,3)){
            ...
            if(invalid[1]){
                ...
                if (getDistance(game->head->x+1,game->head->y,0)>getDistance(game->head->x-1,game->head->y,2)){
